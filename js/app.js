...
// زر المزامنة
    ...
    scanBarcode() {
        // استخدام Web Barcode Detection API
        if (!('BarcodeDetector' in window)) {
            Utils.toast('ماسح الباركود غير مدعوم في هذا المتصفح — أدخل الباركود يدوياً', 'error');
            return;
        }
        ...
        if (navigator.mediaDevices?.getUserMedia) {
            navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
                .then(stream => {
                    // محاولة فتح الكاميرا — في النسخة الكاملة نعرض معاينة هنا
                    stream.getTracks().forEach(t => t.stop());
                    Utils.toast('فعّل وضع تتمة: امسح الباركود بقارئ خارجي أو أدخله يدوياً', '');
                })
                .catch(() => Utils.toast('تعذر الوصول للكاميرا — أدخل الباركود يدوياً', 'error'));
        } else {
            Utils.toast('أدخل الباركود يدوياً أو استخدم قارئاً خارجياً', '');
        }
    },
...
`;
    ...
    saveSettings() {
        ...
        DB.data.settings = { ...DB.data.settings, storeName, currency, phone, lowStockThreshold: lowStock };
        DB.save();
