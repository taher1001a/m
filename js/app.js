...
showAddProductModal(barcode = '') {
        Utils.openModal(`
            ...
            <div class="form-group">
                ...
                <input type="text" id="prodBarcode" value="${barcode}" placeholder="أدخل الباركود أو امسحه بالكاميرا">
            </div>
...
scanBarcode() {
        ...
        this.barcodeScanner.render(
            decodedText => {
                ...
                Utils.closeModal();
                // بعد نجاح المسح نبحث عن المنتج في المخزون
                self.handleScannedBarcode(scanned);
            },
            error => {
     ...
     * - إن وُجد المنتج: يعرضه ويسمح بزيادة الكمية
     * - إن لم يوجد: يفتح نافذة إضافة منتج مع الباركود مملوءاً
     */
    handleScannedBarcode(barcode) {
        ...
        if (existing) {
            // المنتج موجود — نعرض معلوماته مع خيار إضافة كمية
            Utils.openModal(`
                <div class="modal-header">
                    <span class="modal-title">✅ المنتج موجود</span>
                    <button class="modal-close" onclick="Utils.closeModal()">✕</button>
                </div>
                <div class="alert alert-success">
                    المنتج: <strong>${existing.name}</strong><br>
                    الكمية الحالية: <strong>${existing.quantity}</strong><br>
                    السعر: <strong>${Utils.formatMoney(existing.price, DB.data.settings.currency)}</strong>
                </div>
                <div class="form-group">
                    <label>إضافة كمية للمخزون</label>
                    <input type="number" id="existingAddQty" value="1" min="1">
                </div>
                <div class="modal-footer">
                    <button class="btn btn-success" onclick="App.addQtyToExisting('${existing.id}')">إضافة الكمية</button>
                    <button class="btn btn-danger" onclick="Utils.closeModal()">إغلاق</button>
                </div>
            `);
        } else {
            // المنتج غير موجود — نفتح نافذة إضافة منتج مع الباركود مملوءاً
            this.showAddProductModal(barcode);
            Utils.toast('أدخل بيانات المنتج وأكمل الحفظ', '');
        }
    },

    addQtyToExisting(productId) {
        const qty = parseInt(document.getElementById('existingAddQty').value) || 1;
        const p = DB.getProduct(productId);
        if (!p) return;
        DB.updateProduct(productId, { quantity: p.quantity + qty });
        Utils.toast(`تمت إضافة ${qty} إلى ${p.name} ✅`, 'success');
        Utils.closeModal();
        this.renderPage('inventory');
    },
