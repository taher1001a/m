    ...
    async loginWithGoogle() {
        ...
        if (this.firebaseReady) {
            try {
                ...
                // فشل تسجيل الدخول عبر Firebase — نعود تلقائياً للوضع التجريبي
                // حتى يستمر التطبيق بالعمل (السبب غالباً: block popup أو بيئة غير آمنة)
                const demoMode = await this.loginAsDemo();
                hint.textContent = 'تعذّر تسجيل الدخول بجوجل في هذه البيئة — تم الدخول بالوضع التجريبي';
                return demoMode;
            }
        } else {
